...
var node = {
	hash:				props.globals.initNode("/sim/multiplay/generic/string[4]","AAA","STRING"),
...
var iff_hash = {
	...
	loop: func() {
			...
			node.hash.setValue(_calculate_hash(me.time, me.callsign, 1));
	},
};
...
var interrogate = func(tgt) {
	...
	hash1 = _calculate_hash(int(systime()) - int(math.mod(int(systime()),iff_refresh_rate)), tgt.getChild("callsign").getValue(),1);
	hash2 = _calculate_hash(int(systime()) - int(math.mod(int(systime()),iff_refresh_rate)) - iff_refresh_rate, tgt.getChild("callsign").getValue(),1);
